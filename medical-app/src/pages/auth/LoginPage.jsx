...
import { Heart, Mail, Lock, Loader2, Sun, Moon } from 'lucide-react'
...
const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Use response directly — NOT from Redux state
      const role = response.data.user.role
      console.log('Role detected:', role) // ← add this to debug

      if (role === 'DOCTOR') {
        navigate('/doctor')
      } else {
        try { await reminderService.generate() } catch (_) {}
        navigate('/dashboard')
      }
    ...
    <div className="min-h-screen bg-gray-50 dark:bg-gray-950 flex items-center justify-center p-4 relative">
      ...
      <div className="w-full max-w-sm">
        ...
        <div className="text-center mb-8">
          ...
          <h1 className="text-xl font-semibold text-gray-900 dark:text-white">Bienvenue sur MediTrack</h1>
          <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">Connectez-vous à votre espace de santé</p>
        </div>
        ...
        <div className="bg-white dark:bg-gray-900 rounded-2xl border border-gray-200 dark:border-gray-800 p-6 shadow-sm">
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            <div>
              <label className="block text-xs font-medium text-gray-700 dark:text-gray-300 mb-1.5">Adresse email</label>
              <div className="relative">
                <Mail size={15} className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-400" />
                <input
                  type="email" name="email" value={formData.email} onChange={handleChange} required
                  placeholder="exemple@email.com"
                  className="w-full pl-9 pr-4 py-2.5 text-sm border border-gray-200 dark:border-gray-700 rounded-lg bg-white dark:bg-gray-800 text-gray-900 dark:text-white placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-blue-500 transition-colors"
                />
              </div>
            </div>
        ...
        <p className="text-center text-xs text-gray-400 dark:text-gray-600 mt-6">
          MediTrack — Votre santé, notre priorité
        </p>
      </div>
    </div>
  )
